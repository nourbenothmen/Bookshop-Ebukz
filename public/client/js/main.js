...
function updateTotals() {
    ...
    $('.cart_item').each(function() {
        ...
        // mettre à jour le sous-total spécifique
        let livreId = $qtyInput.attr('id').split('-')[1];
        $('#total-' + livreId).text(subTotal.toFixed(2) + ' DT');

        total += subTotal;
    });

    $('#total-general').text(total.toFixed(2) + ' DT'); // met à jour le total général
}

$('.quantity-plus').click(function(e){
    e.preventDefault();
    let $qty = $(this).closest('.quantity').find('.qty-input');
